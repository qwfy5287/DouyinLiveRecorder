...
impl VideoProcessorBuilder {
    ...
    fn build(self) -> VideoProcessor {
        VideoProcessor {
            ...
            thumbnail_generator: Arc::new(self.thumbnail_generator),
            file_renamer: Arc::new(self.file_renamer),
        }
    }
}
...
struct VideoProcessor {
    base_path: PathBuf,
    thumbnail_generator: Arc<Box<dyn ThumbnailGenerator>>,
    file_renamer: Arc<Box<dyn FileRenamer>>,
}
    ...
    fn process_videos(&self) -> std::io::Result<()> {
        ...
        for entry in fs::read_dir(&self.base_path)? {
            ...
            if path.is_file() && path.extension().map(|s| s == video_ext).unwrap_or(false) {
                ...
                let file_renamer = self.file_renamer.clone();
