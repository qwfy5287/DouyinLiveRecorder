...
fn split_video_by_subtitles(draft_content_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for subtitle_segment in subtitle_segments {
        ...
        new_segment["source_timerange"]["start"] = json!(subtitle_start);
        new_segment["source_timerange"]["duration"] = json!(subtitle_end - subtitle_start);
    ...
    for &subtitle_start in &subtitle_start_times {
        if subtitle_start > prev_subtitle_end {
            ...
            new_segment["source_timerange"]["start"] = json!(prev_subtitle_end);
            new_segment["source_timerange"]["duration"] = json!(subtitle_start - prev_subtitle_end);
    ...
    if last_subtitle_end < video_duration {
        ...
        new_segment["source_timerange"]["start"] = json!(last_subtitle_end);
        new_segment["source_timerange"]["duration"] = json!(video_duration - last_subtitle_end);
        new_video_segments.push(new_segment.clone());
    ...
    if !subtitle_start_times.is_empty() {
        ...
        new_segment["source_timerange"]["start"] = json!(0);
        new_segment["source_timerange"]["duration"] = json!(subtitle_start_times[0]);
