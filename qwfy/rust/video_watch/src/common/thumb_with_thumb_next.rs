...
use crate::common::thumb_next::generate_thumbnails;
        ...
        Ok(entries) => {
            for entry in entries.filter_map(Result::ok) {
                ...
                if path.is_file() && path.extension().unwrap_or_default() == video_ext {
                    ...
                    if let Err(e) =
                        generate_thumbnails(Path::new(&input_video_path), &output_pattern)
                    {
...
pub fn process_video(file_path: &Path) {
    ...
    if let Err(e) = generate_thumbnails(file_path, &output_pattern) {
        eprintln!("Error generating thumbnails for '{}': {}", video_name, e);
        return;
    }
