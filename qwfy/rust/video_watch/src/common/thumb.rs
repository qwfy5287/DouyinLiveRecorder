// qwfy/rust/video_watch/src/lib/thumb.rs
            ...
            if path.is_file() {
                ...
                if let Some(capture) = filename.strip_prefix(video_filename).and_then(|f| f.strip_suffix(".jpg")) {
                    ...
                    match frame_number_str.parse::<i32>() {
                        ...
                        Err(e) => {
                            // 错误处理
                            eprintln!("Failed to parse frame number from filename: {}. Error: {}", filename, e);
                            continue; // 跳过当前循环迭代
                        }
                    }
...
pub fn process_video(file_path: &Path) {
    ...
    let output_dir = format!("{}/{}_thumb", file_path.parent().unwrap().to_string_lossy(), video_name);
    let output_pattern = format!("{}/{}_thumb/{}_%03d.jpg", file_path.parent().unwrap().to_string_lossy(), video_name, video_name);
